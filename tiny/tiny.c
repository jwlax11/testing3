...
void doit(int fd)
{
	...
	if (strcasecmp(method, "GET"))
	{
		clienterror(fd, method, "501", "Not Implemented",
				"Tiny does not implement this method");				//line: netp: doit:beginrequesterr
		return;
	...
	printf("Parsed the URI\n");
	if (stat(filename, &sbuf) < 0)
	{
		clienterror(fd, filename, "404", "Not found",
				    "Tiny couldn't find this file");				//line: netp: doit:beginnotfound
		return;
	}																//line: netp: doit:endnotfound

	if (is_static)
	{			/* Serve static content */
		printf("Serving static content\n");
		if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode))
		{
				clienterror(fd, filename, "403", "Forbidden",
					    "Tiny couldn't read the file");				//line: netp: doit:readable
			return;
		}
	...
	{			/* Serve dynamic content */
		printf("Serving dynamic content\n");
		if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode))
		{
				clienterror(fd, filename, "403", "Forbidden",
				       "Tiny couldn't run the CGI program");		//line: netp: doit:executable
			return;
		}
